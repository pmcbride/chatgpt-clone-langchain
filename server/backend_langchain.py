# LangChain Backend API
# File: server/backend_langchain.py
# Description: Like server/backend.py but with a few modifications to support LangChain.
# ==================================================================================================
# Imports
...
class Backend_Api:
    def __init__(self, app, config: dict) -> None:
        self.app = app
        self.chat_model = ChatOpenAI()  # Instantiate the LangChain ChatOpenAI model
    ...
    def _conversation(self):
        try:
            ...
            question = system_message + ' ' + \
                (' '.join(extra) if internet_access else '') + ' ' + prompt

            # Generate the response using the LangChain model
            langchain_resp = self.chat_model.generate_response(question)

            return jsonify(langchain_resp)  # Return the response as a JSON object
        except Exception as e:
            print(f"Error: {e}")
            print(f"Traceback: {e.__traceback__.tb_next}")
            # print(e)
